...
from strategies.lstm_utils import LSTMModel, TimeSeriesScaler, create_batches, prepare_lstm_data
...
SYMBOLS = [
    "acp", "bhw", "brs", "gtc", "jsw", "ker",
    "kgh", "lwb", "peo", "pge", "pkn", "pko", "pzu", "tpe",
]
...
def train_model_for_symbol(symbol: str, df_symbol: pd.DataFrame):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if (epoch + 1) % 50 == 0:
            print(f"Epoch [{epoch+1}/{NUM_EPOCHS}] Train Loss: {epoch_loss/len(X_train):.6f} | Val Loss: {val_loss:.6f}")
