...
OPENAPI_BASE = (os.getenv("SAXO_OPENAPI_BASE") or "https://gateway.saxobank.com/sim/openapi").rstrip("/")
...
class Tokens:
    ...
    def from_token_response(resp: dict[str, Any], code_verifier: str) -> "Tokens":
        ...
        return Tokens(
            ...
            if resp.get("refresh_token_expires_in") else None,
    ...
    def from_json(d: dict[str, Any]) -> "Tokens":
        return Tokens(
            ...
            refresh_exp=float(d.get("refresh_exp")) if d.get("refresh_exp") else None,
...
def login(port: int = 8765) -> Tokens:
    ...
    if not app_url.path:
        raise SystemExit("SAXO_APP_URL must include a path (e.g., http://localhost/oauth/callback).")
    if port in (80, 443):
...
def main() -> None:
    ...
    p_login.add_argument("--port", type=int, default=8765, help="Local callback port (matches APP_URL path)")
    p_login.set_defaults(func=lambda a: login(a.port))
    
    sub.add_parser("ensure", help="Prints a valid access token").set_defaults(func=lambda a: print(ensure_access_token()))  # noqa: T201
